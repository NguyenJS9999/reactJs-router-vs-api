...
export function ProductDetail() {
    ...
    // 1 object item
    const [ stateCurrentProductsDetail,  setCurrentProductsDetail ] = useState( {} );

    // const currentProductsDetail = stateProducts[1];

    // Lấy chuỗi params
    let { slug } = useParams(); console.log('Lấy text là url:', slug); 
    // Lấy id params        
    let urlQueryParameters = window.location.search;
    console.log('Lấy số là id:', urlQueryParameters); 
    
    // Bấm nút chạy hàm hiện item theo url
    function findCurrentProductsDetail () { 
        let item = stateProducts.filter( (product) => product.slug ===  slug )

        setCurrentProductsDetail( item );
        console.log('Bấm nút hiện sản phẩm lọc theo url ', item[0]);
    ...
    useEffect( () => {
        fetch( 'https://rest-api-nodejs-reactjs-router.herokuapp.com/products' ).then( 
          ...
          .then( ( products )  => { 
            ...
            console.log('.then trả về data products:', products) 
          })
    }, []); 

    // Chọn lọc item hiển thị theo param url
    useEffect( () => {
        function findCurrentProductsDetail () { 
            let item = stateProducts.filter( (product) => product.slug ===  slug )

            setCurrentProductsDetail( item );
            console.log('useEffect Tự chạy Hiện sản phẩm lọc theo url', item );
        }
        return findCurrentProductsDetail;
    }, []); 

    console.log('1 object: stateCurrentProductsDetail: ', stateCurrentProductsDetail)

    return (
        <>  
            <Breadcrumb/>
            <button onClick = {findCurrentProductsDetail} >Hiện sản phẩm theo url</button>
            ...
            <div className=" product-detail-image ">
                ...
                <div className="tab-pane fade show    active  " id="product-detail-image-01" role="tabpanel" aria-labelledby="product-detail-image-01-tab">
                    <div className=" product-image ">
                        ...
                        <img src="https://nguyenkeo.github.io/MasterAudio/img/4-acoustic/PCS-318NB-01.png" 
